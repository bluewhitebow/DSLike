    ...
    public class CameraManager : MonoBehaviour
    {
        ...
        public void Tick(float d)
        {
            ...
            if(c_h != 0 || c_v != 0)
            {
                h = c_h;
                v = -c_v;
                targerSpeed = controllerSpeed;
            }
            FollowTarget(d);
            HandleRotations(d, v, h, targerSpeed);
        }
        ...
        void FollowTarget(float d)
        {
            ...
            Vector3 tragetPosition = Vector3.Lerp(transform.position, target.position,speed);
        ...
        void HandleRotations(float d,float v,float h,float targetSpeed)
        {
            if(turnsSmoothing > 0)
            {
                smoothX = Mathf.SmoothDamp(smoothX, h, ref smoothXvelocity, turnsSmoothing);
                smoothY = Mathf.SmoothDamp(smoothY, v, ref smoothYvelocity, turnsSmoothing);
            }
            else
            {
                smoothX = h;
